 ...
 *
 * The cookie carries {username, role, workforceId, iat, exp} and an HMAC-SHA256
...
const SESSION_TTL_SECONDS = 60 * 10; // 10 minutes of inactivity; renewed by authenticated heartbeats while active
...
export async function verifySessionToken(token: string | undefined | null): Promise<SessionPayload | null> {
  ...
  try {
    ...
    if (payload.role !== "owner" && payload.role !== "hr_manager" && payload.role !== "employee") return null;

    // Owners do not map to a workforce record. Employee and HR/Manager
    // sessions must carry a verified workforce id so first-person queries
    // such as "my employees" can be resolved server-side.
    if (payload.role === "owner") {
      payload.workforceId = null;
    } else if (!Number.isInteger(payload.workforceId) || (payload.workforceId as number) < 1) {
      return null;
    }
