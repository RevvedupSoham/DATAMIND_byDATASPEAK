import "server-only";
import { FORBIDDEN_KEYWORDS, FORBIDDEN_KEYWORDS_HR_MANAGER, FORBIDDEN_SYSTEM_TABLES_FOR_NON_OWNER } from "@/lib/sql/schema";
...
// Additionally allowed as a leading statement type for hr_manager (write-enabled) requests.
 ...
 */
function containsParameterPlaceholder(sql: string): boolean {
  let inSingleQuote = false;
  let inDoubleQuote = false;
 ...
 *
 * @param options.allowWrites When true (hr_manager/owner sessions), permits a
 ...
 * privilege-escalation or server-hr_manager operations remain forbidden
 * regardless of this flag — there is no role that unlocks them.
 */
export function validateSql(
  rawSql: unknown,
  options: { allowWrites?: boolean; role?: UserRole } = {}
): ValidationResult {
  const { allowWrites = false, role = allowWrites ? "hr_manager" : "employee" } = options;
  ...
  if (containsParameterPlaceholder(withoutComments)) {
    return {
      ok: false,
      reason: "Parameterized SQL placeholders ($1, $2, etc.) are not supported. Generate a complete executable SQL statement.",
    };
  }

  // GRANT/REVOKE/MERGE/CALL/EXECUTE and the other always-forbidden keywords
  // are checked for every request, hr_manager or not.
  const alwaysForbidden = allowWrites ? FORBIDDEN_KEYWORDS_HR_MANAGER : FORBIDDEN_KEYWORDS;
  ...
  if (role !== "owner") {
    const restrictedTable = FORBIDDEN_SYSTEM_TABLES_FOR_NON_OWNER.find((table) =>
      new RegExp(`\\b${table}\\b`, "i").test(withoutComments)
    );
    if (restrictedTable) {
      return { ok: false, reason: `Access to the DataMind system table \"${restrictedTable}\" is restricted to the Owner.` };
    }
  }

  // Extra guard for hr_manager writes: an UPDATE/DELETE with no WHERE clause
