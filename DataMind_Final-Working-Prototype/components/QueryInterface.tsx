...
export function QueryInterface() {
  ...
  return (
    <section id="ask" className="relative overflow-hidden border-t border-ink-800 bg-ink-950 py-28">
      ...
      <div className="section relative z-10">
          ...
          {role && (
            <span
              className={`rounded-sm border px-3 py-1 text-xs font-semibold uppercase tracking-widest2 ${
                role === "hr_manager" || role === "owner"
                  ? "border-accent-500/40 text-accent-400"
                  : "border-ink-700 text-ink-400"
              }`}
            >
              {role === "owner" ? "Owner · read & write" : role === "hr_manager" ? "HR/Manager · read & write" : "Employee · read only"}
            </span>
          )}
        </div>
        ...
        {status === "error" && error && (
          <div className="alert-danger mt-12 animate-fade-in rounded-sm border px-6 py-6">
            <p className="alert-heading text-sm font-semibold uppercase tracking-widest2">
        ...
        {status === "needs_confirmation" && pending && (
          <div ref={resultRef} className="mt-12 animate-fade-in space-y-6 scroll-mt-28">
            <div className="alert-warning rounded-sm border px-6 py-6">
              <p className="alert-heading text-sm font-semibold uppercase tracking-widest2">
