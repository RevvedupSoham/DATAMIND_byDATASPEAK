...
import { executeReadonlyQuery, DatabaseError, writeAuditLog } from "@/lib/database/supabase";
...
export async function POST(req: NextRequest) {
  ...
  const allowWrites = session.role === "hr_manager" || session.role === "owner";
  ...
  let generated;
  try {
    generated = await generateSqlFromQuestion(question.trim(), session.role, {
      username: session.username,
      role: session.role,
      workforceId: session.workforceId,
    });
  } catch (err) {
  ...
  const validation = validateSql(generated.sql, { allowWrites, role: session.role });
  ...
  //    hr_manager to explicitly confirm — DataMind never silently modifies data
  //    from a single natural-language turn, even for hr_managers.
  ...
  await writeAuditLog({
    ...
    action: "query_read",
    targetType: "query",
    targetIdentifier: validation.sql.slice(0, 160),
    details: { question: question.trim(), rowCount: result.rowCount },
  });
