...
import { executePrivilegedQuery, DatabaseError, writeAuditLog } from "@/lib/database/supabase";
 ...
 * hr_manager) and the SQL itself (via validateSql) rather than trusting the
...
export async function POST(req: NextRequest) {
  ...
  if (session.role !== "hr_manager" && session.role !== "owner") {
    return errorResponse("auth", "Only hr_manager or owner accounts can confirm and run write statements.", 403);
  }
  ...
  // generated statement.
  const validation = validateSql(sql, { allowWrites: true, role: session.role });
  ...
  await writeAuditLog({
    ...
    details: { question: typeof question === "string" ? question : "" },
  });
