...
function LoginForm() {
  ...
  const [panel, setPanel] = useState<UserRole>("employee");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const destination = data?.role === "owner" && (!requestedNext || requestedNext === "/") ? "/owner" : next;
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-ink-950 px-6">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="rounded-sm border border-ink-800 bg-ink-900 p-8">
          <div className="mb-8 grid grid-cols-3 gap-1 rounded-sm border border-ink-700 bg-ink-950 p-1">
            {(["owner", "employee", "hr_manager"] as UserRole[]).map((r) => (
              <button
                key={r}
                type="button"
                onClick={() => setPanel(r)}
              ...
              >
                {r === "owner" ? "Owner" : r === "hr_manager" ? "HR/Manager" : "Employee"}
              </button>
            ))}
          </div>

          {timeout && (
            <div className="mb-5 rounded-sm border border-accent-500/30 bg-accent-500/5 px-4 py-3 text-sm text-ink-300">
              Your DataMind session expired after 10 minutes of inactivity. Please sign in again.
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder={panel === "owner" ? "owner" : panel === "hr_manager" ? "hr_manager" : "employee"}
              />
            </div>
            ...
            <button type="submit" disabled={submitting} className="btn-primary w-full">
              {submitting ? "Signing in…" : `Sign in as ${panel === "owner" ? "Owner" : panel === "hr_manager" ? "HR/Manager" : "Employee"}`}
            </button>
          </form>

          <p className="mt-6 text-center text-xs leading-relaxed text-ink-500">
            {panel === "owner"
              ? "Owners control the DataMind workspace, manage accounts, and can query or change database data."
              : panel === "hr_manager"
                ? "HR/Managers can ask DataMind to change data — create, update, or delete records — in addition to querying it."
                : "Employees can ask DataMind anything about the data, read-only."}
          </p>
        </div>
      </div>
    </div>
