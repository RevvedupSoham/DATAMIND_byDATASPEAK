      ...
      {open && (
        ...
        >
          <div className="border-b border-ink-800 px-4 py-3">
            ...
            <p className={`mt-0.5 text-xs uppercase tracking-widest2 ${session.role === "owner" || session.role === "hr_manager" ? "text-accent-400" : "text-ink-500"}`}>
              {session.role === "owner" ? "Owner" : session.role === "hr_manager" ? "HR/Manager" : "Employee"}
            </p>
          </div>
          {session.role === "owner" && (
            <button
              type="button"
              role="menuitem"
              onClick={() => { setOpen(false); router.push("/owner"); }}
              className="block w-full px-4 py-2.5 text-left text-sm text-ink-300 transition-colors hover:bg-ink-800 hover:text-ink-100"
            >
              Owner Dashboard
            </button>
          )}
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${
        scrolled ? "bg-ink-950/90 backdrop-blur-sm shadow-[0_1px_0_0_rgb(var(--grid-line)/0.06)]" : "bg-transparent"
      }`}
    >
        ...
        <ul className="hidden items-center gap-10 md:flex">
          {LINKS.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="group relative text-xs font-semibold uppercase tracking-widest2 text-ink-300 transition-colors hover:text-accent-400"
              >
                {link.label}
